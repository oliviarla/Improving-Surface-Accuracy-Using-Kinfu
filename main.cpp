...
// 모든 점이 유효한 projection을 갖는 단위 평면에서 conservative(?) 바운딩 박스 계산
...
int main(int argc, char** argv)
{
    ...
    // Calibration한 결과값들을 distCoeffs 행렬에 저장
    // k1, k2: radial distortion계수 p1, p2: tangential distortion 계수
    ...
    while (!stop && !visualization.wasStopped())
    {
        ...
        else if (key == 'w') //write mode일 경우 ply파일로 저장
        {
